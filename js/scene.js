...
export function build3DScene(mountEl, isMobile) {
  ...
  renderer.setPixelRatio(
    Math.min(window.devicePixelRatio, isMobile ? 1.35 : 1.75)
  );
  ...
  const deskTop = new THREE.Mesh(
    new THREE.BoxGeometry(4.9, 0.01, 3.25),
    new THREE.MeshStandardMaterial({
      color: 0x8d5d35,
      roughness: 0.55,
      metalness: 0.08
  ...
  tower.position.set(2.05, 0.38, 0.62);
  tower.castShadow = true;

  deskGroup.add(tower);

  const deskLight = new THREE.PointLight(
    0xffd28a,
    0.6,
    5
  );
  ...
  chairGroup.position.set(0, 0.05, 1.9);
  ...
  const chairBack = new THREE.Mesh(
    new THREE.BoxGeometry(0.81, 0.98, 0.42),
    new THREE.MeshStandardMaterial({
      color: 0x3f2e23,
      roughness: 0.9,
      metalness: 0.18
  ...
  [
    ...
    const leg = new THREE.Mesh(
      new THREE.BoxGeometry(0.048, 0.82, 0.09),
      chairLegMaterial
    );
  ...
  const chairArmLeft = new THREE.Mesh(
  ...
  function createMonitor(
    ...
    ctx.fillText(
      'UI / UX | RESPONSIVE | CONVERSION',
      140,
      340
    );

    ctx.fillText(
      'GITHUB | DATABASE | COMMUNICATION',
      160,
      440
    );
    ...
    const screen = new THREE.Mesh(
      new THREE.BoxGeometry(1.68, 1.04, 0.06),
      screenMat
    );
    ...
    const bezel = new THREE.Mesh(
      new THREE.BoxGeometry(1.8, 1.16, 0.08),
      new THREE.MeshStandardMaterial({
        color: 0x191919,
        roughness: 0.7,
        metalness: 0.1
    ...
    const stand = new THREE.Mesh(
      new THREE.BoxGeometry(0.26, 0.28, 0.16),
      new THREE.MeshStandardMaterial({
        color: 0x242424,
        roughness: 0.55,
        metalness: 0.1
    ...
    const base = new THREE.Mesh(
      new THREE.BoxGeometry(0.52, 0.06, 0.42),
      new THREE.MeshStandardMaterial({
        color: 0x343434,
        roughness: 0.65,
        metalness: 0.08
  ...
  const monitorLeft = createMonitor(
    -1.25,
    1.28,
    -0.4,
    0.18,
  ...
  const monitorRight = createMonitor(
    1.25,
    1.25,
    -0.25,
    -0.16,
  ...
  monitorLeft.group.position.y =
    1.28 + Math.sin(t * 1.2) * 0.008;

  monitorRight.group.position.y =
    1.25 + Math.cos(t * 1.2) * 0.008;

  monitorLeft.group.rotation.y =
    0.18 + Math.sin(t * 0.7) * 0.006;

  monitorRight.group.rotation.y =
    -0.16 + Math.cos(t * 0.7) * 0.006;

  deskLight.intensity =
    0.55 + Math.sin(t * 2.0) * 0.04;

  rimLight.intensity =
    0.24 + Math.sin(t * 1.4) * 0.02;
